"""
Another set of tests focusing on attention vs latte vs scan attention.
Also check einops vs matmul.
This is the most clean and accurate benchmark file
"""
...
######################################## Einops vs Matmul Test ########################################
@partial(jax.jit, static_argnums=(3,))
def einops(Wq, Wk, Wv, H, X):
    B, T, C = X.shape
    L = Wk.shape[-1]
    head_dim = C // H
...
@partial(jax.jit, static_argnums=(3,))
def matmul(Wq, Wk, Wv, H, X):
    B, T, C = X.shape
    L = Wk.shape[-1]
    head_dim = C // H

    V = (X @ Wv).transpose(1, 2, 0).reshape(T, H, head_dim, B)
    Q = (X @ Wq).transpose(1, 2, 0).reshape(T, H, L // H, B)
    K = (X @ Wk).transpose(1, 2, 0).reshape(T, H, L // H, B)
    return K


def test_einops(B=60, T=20000, C=512, L=128, H=4, repeats=100):
    device = "cuda"
    # Random state
    ...
    Wq, Wk, Wv = (
        jax.random.normal(key_q, (C, H * L)),
        jax.random.normal(key_k, (C, H * L)),
        jax.random.normal(key_v, (C, C)),
    )
    bias = jnp.tril(jnp.ones(shape=(T, T))).reshape(1, 1, T, T)
    X = jax.random.normal(key_x, (B, T, C))

    out0 = einops(Wq, Wk, Wv, H, X).block_until_ready()
    out1 = matmul(Wq, Wk, Wv, H, X).block_until_ready()

    einops_ = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        out3 = einops(Wq, Wk, Wv, H, X).block_until_ready()
        end = time.perf_counter()
        einops_[i] = (end - start) * 1000

    matmul_ = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        out4 = matmul(Wq, Wk, Wv, H, X).block_until_ready()
        end = time.perf_counter()
        matmul_[i] = (end - start) * 1000

    print(f"Min Einops ms: {min(einops_)}, Mean Einops ms: {np.mean(einops_)}")
    print(f"Min Matmul ms: {min(matmul_)}, Mean Matmul ms: {np.mean(matmul_)}")
...
def scan_latte(Wq, Wk, Wv, H, X, unroll=100):
    def accumulate(carry, args):  #
        ...
        # carry = carry + jnp.expand_dims(K_exp_t, 1) @ jnp.expand_dims(V_t, 0) # (L, D)
...
def stable_scan_latte(Wq, Wk, Wv, H, X, unroll=100):
    ...
    # jax.debug.print(f"Qs: {Qs.shape}, K: {K.shape}, V: {V.shape} maxi: {maxi.shape}")
    _, y = jax.lax.scan(
        accumulate,
        unroll=unroll,
...
def ablation_T(B=2, T=10000, C=512, L=512, H=4, repeats=100, limit_att=100000):
    ...
    stable_scan_ = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        _ = stable_scan_latte(Wq, Wk, Wv, H, X, unroll=UNROLL).block_until_ready()
        end = time.perf_counter()
        stable_scan_[i] = (end - start) * 1000

    mem_inef_ = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        _ = mem_ineff_latte(Wq, Wk, Wv, H, X).block_until_ready()
        end = time.perf_counter()
        mem_inef_[i] = (end - start) * 1000

    bid_latt_ = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        _ = bid_latte(Wq, Wk, Wv, H, X).block_until_ready()
        end = time.perf_counter()
        bid_latt_[i] = (end - start) * 1000

    if T <= limit_att:
        self_att_ = [0.0] * repeats
        for i in range(repeats):
            start = time.perf_counter()
            _ = causal_self_attention(QKV, X, bias, H).block_until_ready()
            end = time.perf_counter()
            self_att_[i] = (end - start) * 1000

    google_lin_ = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        _ = google_linmem_wrp(QKV, X, H, query_chunk_size=qchz).block_until_ready()
        end = time.perf_counter()
        google_lin_[i] = (end - start) * 1000

    print(
        f"Min Mem Ineff ms: {min(mem_inef_)}, Mean Mem Ineff ms: {np.mean(mem_inef_)}"
    )
    print(
        f"Min Bid Latte ms: {min(bid_latt_)}, Mean Bid Latte ms: {np.mean(bid_latt_)}"
    )
    print(
        f"Min Stabel Scan ms: {min(stable_scan_)}, Mean Stabel Scan ms: {np.mean(stable_scan_)}"
    )
    if T <= limit_att:
        print(
            f"Min SelfAtt ms: {min(self_att_)}, Mean SelfAtt ms: {np.mean(self_att_)}"
        )
    print(
        f"Min GoogleLin ms: {min(google_lin_)}, Mean GoogleLin ms: {np.mean(google_lin_)}"
    )
    if T <= limit_att:
        self_att_mean = np.mean(self_att_)
    else:
        self_att_mean = float("inf")
    return {
        "stable_scan": np.mean(stable_scan_),
        "mem_ineff": np.mean(mem_inef_),
        "bid_latte": np.mean(bid_latt_),
        "goog_lin": np.mean(google_lin_),
        "self_attention": self_att_mean,
    }


def ablation_L(B=2, T=2000, C=512, H=4, repeats=100):
    # Batch size is fixed to 1 for now (nned to modify google att)
    qchz = T // 2
    master_key = jax.random.PRNGKey(0)
    master_key, key_abl = jax.random.split(master_key)
    master_key, key_qkv, key_q, key_k, key_v, key_x = jax.random.split(
        master_key, num=6
    )
    L = C
    ...
    print("-----" * 10)
    print(X.shape)
    # dummy call for jit compile
    out_mem_ineff = mem_ineff_latte(Wq, Wk, Wv, H, X).block_until_ready()
    out_google = google_linmem_wrp(QKV, X, H, query_chunk_size=qchz).block_until_ready()
    bias = jnp.tril(jnp.ones(shape=(T, T), dtype=bool)).reshape(1, 1, T, T)
    out_causal = causal_self_attention(QKV, X, bias, H).block_until_ready()

    out_stable_scan = stable_scan_latte(Wq, Wk, Wv, H, X, unroll=10).block_until_ready()
    out_bid = bid_latte(Wq, Wk, Wv, H, X).block_until_ready()

    jax.debug.print(
        "Stable Latte OK: {}",
        jnp.all(jnp.isclose(out_stable_scan, out_mem_ineff, rtol=1e-3, atol=1e-3)),
    )
    ...
    for i in range(repeats):
        ...
        self_att_[i] = (end - start) * 1000
    outputs["causal_att"] = np.mean(self_att_)

    goog_mem = [0.0] * repeats
    for i in range(repeats):
        start = time.perf_counter()
        _ = google_linmem_wrp(QKV, X, H, query_chunk_size=qchz).block_until_ready()

        end = time.perf_counter()
        goog_mem[i] = (end - start) * 1000
    outputs["google_mem"] = np.mean(goog_mem)

    outputs["causal_latte"] = OrderedDict()
    outputs["bid_latte"] = OrderedDict()
    for L in range(64, 514, 64):  # [16, 32, 64, 128, 256, 512]: #
        key_q, key_k, key_v, key_abl = jax.random.split(key_abl, num=4)
        Wq, Wk, Wv = (
            jax.random.normal(key_q, (C, L)),
            jax.random.normal(key_k, (C, L)),
            jax.random.normal(key_v, (C, C)),
        ...
        # dummy call
        out_stable_scan = stable_scan_latte(
            Wq, Wk, Wv, H, X, unroll=10
        ).block_until_ready()
        out_bid = bid_latte(Wq, Wk, Wv, H, X).block_until_ready()
        ...
        for i in range(repeats):
            start = time.perf_counter()
            _ = stable_scan_latte(Wq, Wk, Wv, H, X, unroll=10).block_until_ready()
            end = time.perf_counter()
            tmp[i] = (end - start) * 1000
        outputs["causal_latte"][L] = np.mean(tmp)
        ...
        for i in range(repeats):
            ...
            tmp[i] = (end - start) * 1000
        outputs["bid_latte"][L] = np.mean(tmp)
...
def main_ablation_T():
    ...
    for T in [1000, 3000, 5000, 7000, 10000, 12000, 14000, 16000]:
        time_ms = ablation_T(B=2, T=T, C=128, L=128, H=4, repeats=100, limit_att=17000)
        res[T] = time_ms
    print(res)
    with open(os.path.join(DATA_DIR, "benchm_128.json"), "w+", encoding="utf-8") as f:
        json.dump(res, f)


def main_ablation_L():
    res = ablation_L(B=4, T=5000, C=512, H=4, repeats=100)
    print(res)
    with open(os.path.join(DATA_DIR, "benchm_L_512.json"), "w+", encoding="utf-8") as f:
        json.dump(res, f)
